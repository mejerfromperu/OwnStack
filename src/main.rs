...
impl <T> Stack<T> where T : Debug {
    ...
    fn printf(mut self) -> Self {
        ...
        for _i in 0..self.vec.len() as i32 {
            ...
            println!("{:?}", placeholder);
            if placeholder.is_some() {buffer.push(placeholder.unwrap());}
        ...
        for _i in 0..buffer.len() as i32 {
            ...
            println!("{:?}", placeholder);
            if placeholder.is_some() { let _ = self.push(placeholder.unwrap());}
    ...
    fn new (size: u8) -> Self { // Instantiér struct med den bestemte kapacitet,
...
fn test_using_new() {
    ...
    let first_push:Result<i32> = match new_stack.push("Hello") {
        Ok(val) => {Ok(val)},
        Err(_) => {Err(Error::new(ErrorKind::StorageFull, "Error pushing element onto stack; it's full"))}
    };
